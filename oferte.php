...
$oferte 	  = $offers->get_offers($_SESSION['id'],$stat);
$oferte_count = count($oferte);
?>
...
<!-- CONTENT BEGIN -->
        <div id="content">
        	...
        	<div class="span-23 span-container">
                <p>Dati click pe oferte pentru mai multe detalii.</p>
                <table id="cereri">
                	<thead>
                ...
                foreach ($oferte as $oferta) {
					$user_req 		= $users->userdata($oferta['user_id']);

					?>
                    <tr class="align-center">
                        <td><a href="request.php?id=<?php echo $oferta['request_id']; ?>"><?php echo $oferta['id'];?></a></td>
                        <td><a href="request.php?id=<?php echo $oferta['request_id']; ?>"><?php echo $oferta['request_id'];?></a></td>
                        <td>
                        	<a href="request.php?id=<?php echo $oferta['request_id']; ?>">
							<?php
								echo $user_req['company'];
							?>
                            </a>
                        </td>
                        <td><a href="request.php?id=<?php echo $oferta['request_id']; ?>"><?php echo substr($oferta['file'],17);?></a></td>
                        <td>
                        	<a href="request.php?id=<?php echo $oferta['request_id']; ?>">
								<?php
									if($oferta['status'] == 0) echo 'In asteptare';
									if($oferta['status'] == 1) echo 'Acceptata';
									if($oferta['status'] == 2) echo 'Refuzata';
								?>
                        	</a>
                        </td>
                    </tr>
