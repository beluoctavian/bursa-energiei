                ...
                if (!empty($_POST))
                {
                    foreach ( $_POST as $key => $value )
                    {
                        if ( ( !is_string($value) && !is_numeric($value) ) || !is_string($key) )
                            continue;
						$news_emails 		= $users->get_newsletter_mails();
						$news_emails_count = count($news_emails);
					
						$headers = "From: " . "Bursaenergiei.ro<office@bursaenergiei.ro>" . "\r\n";
						$headers .= "Reply-To: ". "Bursaenergiei.ro<office@bursaenergiei.ro>" . "\r\n";
						$headers .= "CC: office@bursaenergiei.ro\r\n";
						$headers .= "MIME-Version: 1.0\r\n";
						$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";
						
						$value = $_POST['editor'];
                        if ( get_magic_quotes_gpc() ){
							foreach ($news_emails  as $nemail ) {
								mail($nemail['email'],'Newsletter',$value,$headers);
							}
                        }
                        else{
                            $value = (string)$value;
