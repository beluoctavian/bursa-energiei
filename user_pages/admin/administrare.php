...
if (isset($_POST['submit'])) {
	...
	if($nr_locatii == 0){
		...
		$errors[] = 'Trebuie sa adaugati neaparat un fisier pentru upload.';
	...
	{	
		if($_FILES['fisier']['name'][$i] != '')
			...
			else
			{
				...
				while(file_exists("../../uploads/" . $nume))
					$nume = rand(0,9999999).$_FILES['fisier']['name'][$i];
				
				if(!move_uploaded_file($_FILES["fisier"]["tmp_name"][$i],"../../uploads/" . $nume))
					$errors[] = $_FILES['fisier']['name'][$i].' nu s-a putut uploada. Size: '.($_FILES['fisier']['size'][$i] / 1048576).' MB';
...
<script>
	...
	$( document ).ready( function() {
		$( '#despre' ).ckeditor(); // Use CKEDITOR.replace() if element is <textarea>.
		$( '#editable' ).ckeditor(); // Use CKEDITOR.inline().
	} );

	function setValue() {
		$( '#despre' ).val( $( 'input#val' ).val() );
	}
...
<!-- CONTENT BEGIN -->
        <div id="content">
        	<div class="span-23 float-left">
                <h3>Administrare continut website</h3>
                <div class="span-container">
					<?php
                        if (isset($_POST['submit']) && empty($errors)) {
                            echo '<div class="error">Locatia fisierului uploadat este: ' . $absolute_path . '/uploads/' . $nume . '</div>';	
                        }
                    ?>
                    ...
                    <hr />
                	<div>Pagini:</div>
                	<a class="simple_button float-left" href="administrare_despre_noi.php">Despre noi</a>
                	<a class="simple_button float-left" href="administrare_schimbare_furnizor.php">Schimbare furnizor</a>
                	<a class="simple_button float-left" href="administrare_legislatie.php">Legislatie</a>
                	<a class="simple_button float-left" href="administrare_intrebari.php">Intrebari</a>
                	<a class="simple_button float-left" href="administrare_reclame.php">Reclame</a>
                	<a class="simple_button float-left" href="create_newsletter.php">Newsletter</a>
                	<a class="simple_button float-left" href="administrare_banner.php">Banner</a>
