...
<!-- CONTENT BEGIN -->
        <div id="content">
        	<div class="span-23 float-left">
                <h3 class="title">Schimbarea furnizorului de energie electrica</h3>
                <div class="span-container">
                    <?php
					$query = $db->prepare("SELECT `content` FROM `pages` WHERE `name` = ?");
					$query->bindValue(1, 'schimbare_furnizor');
					$query->execute();
					$schimbare_furnizor = $query->fetch(PDO::FETCH_ASSOC);
					$text = html_entity_decode($schimbare_furnizor['content']);
					echo $text;
					?>
                </div>
            </div>
